...
INSERT INTO map_configs.controls(control_id,js,abstract) VALUES ('sharemap#1','/* Log mapstate use */ window.onload = function() { const mapStateId = urlParams.get("mapStateId"); if (mapStateId) { fetch("./mapstateLog/mapstateLog-loader.php", { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: "mapStateId=" + encodeURIComponent(mapStateId) }) .then(response => { if (!response.ok) { console.error("Failed to log mapStateId"); } }) .catch(error => { console.error("Error:", error); }); } };','Skapar en delbar länk till kartan. Aktuell utbredning och zoom, synliga lager och kartnålen (om tillämpligt) kommer att delas. Om ett objekt på kartan väljs, kommer objektets ID att finnas i länken som gör att kartan zoomar in på den när den laddas. Detta gäller för WFS, Geojson, Topojson och AGS Feature lager. Sharemap-kontrollen kommer också med möjlighet att spara karttillstånd på servern (kräver Origo Server). Ett sparat karttillstånd hämtas med ett ID istället för en URL.');
...
INSERT INTO map_configs.helps(help_id,abstract) VALUES ('control:js','<b>Kontroll > JS</b><br>Javascript-kod som inkluderas direkt i kartans html-dokument (placeras efter kartspecifik JS). Förutom variabeln "origo" så är även konstanten "urlParams" redan definierade.');
...
INSERT INTO map_configs.helps(help_id,abstract) VALUES ('plugin:css','<b>Plugin > CSS</b><br>CSS-kod som inkluderas direkt i kartans html-dokument (placeras efter kartspecifik CSS).');
...
INSERT INTO map_configs.helps(help_id,abstract) VALUES ('plugin:js','<b>Plugin > JS</b><br>Javascript-kod som inkluderas direkt i kartans html-dokument (placeras efter kartspecifik JS och JS från kontroller). Förutom variabeln "origo" så är även konstanten "urlParams" redan definierade.<br><br><b>Exempel för Swiper-plugin:</b><br>origo.on(''load'', function (viewer) {<br>&emsp;const swiper = Swiper({<br>&emsp;&emsp;origoConfig: ''../index.json'',<br>&emsp;&emsp;circleRadius: 100,<br>&emsp;&emsp;alwaysOnTop: true,<br>&emsp;&emsp;initialLayer: ''orto016'',<br>&emsp;&emsp;initialControl: ''swipe'',<br>&emsp;&emsp;showLayerListOnStart: true,<br>&emsp;&emsp;tooltips: {<br>&emsp;&emsp;&emsp;swiper: ''Jämför kartvyer'',<br>&emsp;&emsp;&emsp;swipeBetweenLayers: ''Jämför sida-sida'',<br>&emsp;&emsp;&emsp;circleSwipe: ''Jämför med kikhål'',<br>&emsp;&emsp;&emsp;layerList: ''Välj lager från lista''<br>&emsp;&emsp;}<br>&emsp;});<br>&emsp;viewer.addComponent(swiper);<br>});');
