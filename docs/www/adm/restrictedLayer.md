...
| `userAuthorized.php` (⚠️ mapp saknas i tidigare filträd, ligger troligen direkt i `functions/restrictedLayer/` eller i `common/` – bör bekräftas) | `userAuthorized($user, $restrictedLayer): bool` | Avgör om en given användare är behörig till ett specifikt skyddat lager, baserat på `authorized_users` (id-matchning) eller `authorized_groups` (någon gemensam grupp) |
